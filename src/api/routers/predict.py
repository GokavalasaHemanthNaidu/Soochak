...
async def counterfactual(
    ...
    if "Speed_Limit" in modified_input and (
        modified_input["Speed_Limit"] < 0 or modified_input["Speed_Limit"] > 200
    ):
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
    ...
    """,
        (
            ...
            str(
                incident_dict.get(
                    feat_lower if feat_lower != "junction_control" else "junction_ctrl", ""
                )
            ),
            request.new_value,
            original_prob,
