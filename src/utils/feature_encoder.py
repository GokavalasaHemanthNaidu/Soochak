...
def encode_features(data: dict) -> np.ndarray:
    ...
    encoded = {}
    for feat in ["Road_Type", "Weather", "Lighting", "Junction", "Junction_Control",
    ...
    road_geom_idx = (encoded["Road_Type"] * 0.3 + encoded["Junction"] * 0.2 +
                     encoded["Junction_Control"] * 0.2 + encoded["Lighting"] * 0.3)
    
    vector = [
        encoded["Road_Type"], float(data.get("Speed_Limit", 50)),
        encoded["Weather"], encoded["Lighting"], encoded["Junction"],
        encoded["Junction_Control"], encoded["Vehicle_Type"], encoded["Driver_Age"],
        encoded["Urban_Rural"], encoded["State"], encoded["City"],
        state_risk, weather_risk, road_geom_idx,
    ]
