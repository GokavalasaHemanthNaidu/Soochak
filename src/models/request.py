...
class CounterfactualRequest(BaseModel):
    ...
    feature_to_change: str = Field(
        ..., description="Feature name to change (e.g. 'Speed_Limit', 'Weather')"
    )
    new_value: str = Field(..., description="New value for the feature")

    class Config:
        json_schema_extra = {
            "example": {"incident_id": 1, "feature_to_change": "Speed_Limit", "new_value": "40"}
        }
...
class ExplainRequest(BaseModel):
    ...
    class Config:
        json_schema_extra = {"example": {"prediction_id": 1}}


class FeedbackRequest(BaseModel):
    prediction_id: int = Field(..., description="SQLite row ID from predictions table")
    was_correct: Optional[int] = Field(
        None, ge=0, le=1, description="1 if prediction was correct, 0 if wrong"
    )
