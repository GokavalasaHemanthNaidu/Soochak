...
async def test_indexes_exist(test_db):
    ...
    expected_indexes = [
        "idx_predictions_incident",
        "idx_incidents_datetime",
        "idx_incidents_state",
        "idx_incidents_city",
        "idx_predictions_created",
        "idx_feedback_prediction",
...
async def test_foreign_keys(test_db):
    ...
    async with test_db.execute(
        "SELECT COUNT(*) as count FROM predictions WHERE incident_id=1"
    ) as cursor:
    ...
    with pytest.raises(aiosqlite.IntegrityError):
        await test_db.execute(
            "INSERT INTO predictions (incident_id, predicted_class) VALUES (999, 1)"
        )
        await test_db.commit()
